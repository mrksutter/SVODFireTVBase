...
public class VideoProvider {
    ...
    public static HashMap<String, List<Movie>> buildMedia(Context ctx, String url)
        ...
        if (null != categories) {
            ...
            for (int catIdx = 0; catIdx < categoryLength; catIdx++) {
                ...
                //Log.d("THE CATEGORY NAME IS:",categoryName);
                ...
                if (null != videos) {
                    for (int vidIdx = 0, vidSize = videos.length(); vidIdx < vidSize; vidIdx++) {
                        ...
                        runtime = video.getString(TAG_RUNTIME);
                        Log.d("RUNTIME IS", runtime);
                       //videoUrl = getVideoPrefix(categoryName, getVideoSourceUrl(videoUrls));
                        videoUrl = getVideoSourceUrl(videoUrls);
                        //bgImageUrl = getThumbPrefix(categoryName, title, video.getString(TAG_BACKGROUND));
                        bgImageUrl = video.getString(TAG_BACKGROUND);
                       // cardImageUrl = getThumbPrefix(categoryName, title, video.getString(TAG_CARD_THUMB));
                        ...
                        movie = buildMovieInfo(categoryName, title, description, studio,
                                videoUrl, cardImageUrl, bgImageUrl, runtime);
    ...
    private static Movie buildMovieInfo(String category,
        ...
        movie.setVideoDuration(runtime);
        Log.d(TAG, "THE RUNTIME IS: " + runtime);
